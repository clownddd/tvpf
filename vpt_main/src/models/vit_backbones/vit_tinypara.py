...
class Mlp_small(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = Linear(768, 64) # can be 8,32
        self.fc2 = Linear(64, 768) # can be 8,32
        self.act_fn = ACT2FN["gelu"]
        self.dropout = Dropout(config.transformer["dropout_rate"])
...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        self.inter_dim = 64 # can tune to best
        ...
        self.internal_correlation = InternalCorrelation()
    ...
    def forward(self, hidden_states, prompt_states): # jdg
        ...
        for i in range(12):
            if i in p2b_list:
                # 对提示词进行内部运算
                prompt_states = self.internal_correlation(prompt_states)
...
class InternalCorrelation(nn.Module):
    def __init__(self):
        super(InternalCorrelation, self).__init__()
