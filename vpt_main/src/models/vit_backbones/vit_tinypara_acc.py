...
# spatially aligned mode
kv_list = list()
for i in range(1,15):
    for j in range(1,15):
        kv_list.append([i*16+j-17,i*16+j-16,i*16+j-15,i*16+j-1,i*16+j,i*16+j+1,i*16+j+15,i*16+j+16,i*16+j+17])
...
class CrossAttention(nn.Module):
    def __init__(self, dim = 768, heads = 1, dim_head = 768, dropout = 0.1): # vit base - dim=768
    ...
    def forward(self, x, y_q):
        B, N, C = x.shape
        q = self.to_q(y_q) # B, N, C
        k = self.to_k(x) # B, N, C
        v = self.to_v(x) # B, N, C
        
        # deal cls token
        dots = einsum('b i d, b j d -> b i j', q[:, [0], :], k) * self.scale
        attn = dots.softmax(dim=-1)
        cls_out = einsum('b i j, b j d -> b i d', attn, v) # B, 1, C
        
        # deal other
        q = q[:,1:,:].unsqueeze(2).flatten(0,1) # B, N, 1, C
        k = k[:,1:,:].transpose(1, 2).reshape(B,C,14,14) # B, C, H, W
        k = nn.ZeroPad2d(padding=1)(k).flatten(2).transpose(1, 2) # B, N*, C
        v = v[:,1:,:].transpose(1, 2).reshape(B,C,14,14) # B, C, H, W
        v = nn.ZeroPad2d(padding=1)(v).flatten(2).transpose(1, 2) # B, N*, C
        
        # k v: B, N, 9, C
        k = k[:,kv_list,:].flatten(0,1) # BN 9 C
        v = v[:,kv_list,:].flatten(0,1)

        attn = torch.bmm(q, k.transpose(1, 2)) * self.scale
        attn = attn.softmax(dim=-1)
        other_out = torch.bmm(attn, v).squeeze()
        other_out = other_out.reshape(B,196,C)
        x = torch.cat([cls_out,other_out], dim=1)
