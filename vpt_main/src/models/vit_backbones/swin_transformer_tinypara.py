...
class CrossAttention(nn.Module):
    def __init__(self, dim = 512, heads = 16, dim_head = 32, dropout = 0.1): # vit base - dim=768
    ...
    def forward(self, x_qkv, y_q):
        ...
        qkv = self.qkv(y_q).reshape(b, n, 3, h, _ // h).permute(2, 0, 3, 1, 4)
        q = qkv[0]
        
        qkv = self.qkv(x_qkv).reshape(b, n, 3, h, _ // h).permute(2, 0, 3, 1, 4)
        k,v = qkv[1],qkv[2]
        ...
        q_list = list()
        kv_list = list()
        top_edge = [i for i in range(1, 13)]
        down_edge = [i for i in range(183, 195)]
        left_edge = [i for i in range(14, 169, 14)]
        right_edge = [i for i in range(27, 182, 14)]

        for i in range(0, 196):
            q_list.append(i)
            # corner
            if i == 0:
                kv_list.append([0, 1, 14, 15])
            elif i == 13:
                kv_list.append([12, 13, 26, 27])
            elif i == 182:
                kv_list.append([168, 169, 182, 183])
            elif i == 195:
                kv_list.append([180, 181, 194, 195])
            elif i in top_edge:
        ...
        for patch in q_list:
            dots = einsum('b h i d, b h j d -> b h i j', q[:, :, [patch], :], k[:, :, kv_list[patch], :]) * self.scale
            attn = dots.softmax(dim=-1)
            out = einsum('b h i j, b h j d -> b h i d', attn, v[:, :, kv_list[patch], :])
            out = rearrange(out, 'b h n d -> b n (h d)')
            if patch == 0:
                new_out = out
            else:
...
class Cross_Block(nn.Module):
    def __init__(self):
        ...
        self.ffn_norm = LayerNorm(512, eps=1e-6)
...
class Mlp_small(nn.Module):
    def __init__(self):
        ...
        self.fc1 = Linear(512, 8) #========adjust according datasets
        self.fc2 = Linear(8, 512)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, p, deep_prompt_embd=None):
        ...
        if not self.deep_prompt:
            for i in range(len(self.blocks)):
                if self.use_checkpoint:
                    ...
                    if self.depth==18:
                        if i % 6 == 0:
                            fusion = self.cross_attn[i](x, p)
                            x = self.blocks[i](x)
                            p = self.blocks[i](p)
                            p = p+fusion*self.add_scale
                        else:
                            fusion = self.cross_attn[i](p, x)
                            x = self.blocks[i](x)
                            p = self.blocks[i](p)
                            x = x+fusion*self.add_scale
                    else:
                        x = self.blocks[i](x)
                        p = self.blocks[i](p)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # add prompt for swin
        self.inter_dim = 8
        self.deep_ppt = nn.Parameter(torch.zeros(1, 3136, self.inter_dim)) # car is 1 other is 2
        self.proj_ppt = nn.Linear(self.inter_dim,128) # input 128
        trunc_normal_(self.deep_ppt, std=0.02)
        nn.init.xavier_uniform_(self.proj_ppt.weight)
        nn.init.normal_(self.proj_ppt.bias, std=1e-6)
    ...
    def forward(self, x):
        ...
        # for prompt
        p = self.proj_ppt(self.deep_ppt).expand(B, -1, -1)
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        x, ppt = self.patch_embed(x)
        if self.ape:
            x = x + self.absolute_pos_embed
            ppt = ppt + self.absolute_pos_embed
        x = self.pos_drop(x)
        ppt = self.pos_drop(ppt)

        for i in range(len(self.layers)):
            x, ppt = self.layers[i](x, ppt)
            # x = layer(x)
            # logger.info(x.shape)
        ...
        ppt = self.norm(ppt)  # B L C
        ppt = self.avgpool(ppt.transpose(1, 2))  # B C 1
        ppt = torch.flatten(ppt, 1)
        return x, ppt
